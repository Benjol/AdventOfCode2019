...
type InputOutput = { Input : unit -> int64; Output : int64 -> unit }
...
let input = System.IO.File.ReadAllText(@"C:\Temp\Day9.txt").Split([|','|]) |> Array.map int64
let list = System.Collections.Generic.List<int64>()
let io = { Input = (fun () -> 1L); Output = list.Add }
run input io
list.ForEach(fun x -> printfn "%A" x)
